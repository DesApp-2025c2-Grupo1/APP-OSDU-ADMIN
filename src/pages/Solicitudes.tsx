...
export function Solicitudes() {
  ...
  return (
    <div className="w-full p-6 space-y-6">
      {/* Título */}
      <div>
        <h1 className="text-2xl font-black text-gray-900">Solicitudes de Prestadores</h1>
        <p className="text-sm text-gray-400 mt-1">
          Revisá y resolvé las solicitudes enviadas por los prestadores.
        </p>
      </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {solicitudes.map((s) => {
                    const badgeCls = ESTADO_CLS[s.estado] ?? "bg-gray-100 text-gray-600";
                    return (
                      <tr
                        key={s.id}
                      ...
                      >
                        <td className="px-6 py-4">
                          ...
                          {s.prestadorCuit && (
                            <p className="text-[10px] text-gray-400 font-mono mt-0.5">{s.prestadorCuit}</p>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <p className="font-bold text-gray-900">{s.afiliado ?? "—"}</p>
                          {s.credencial && (
                        ...
                        <td className="px-6 py-4">
                          <p className="font-bold text-gray-900">{s.fecha}</p>
                        </td>
