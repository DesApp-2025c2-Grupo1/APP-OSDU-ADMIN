...
export function GrupoFamiliar() {
  ...
  const handleAgregarFamiliar = () => {
    setShowAddFamiliarPopup(true);
  };

  const handleSaveFamiliar = (nuevoFamiliar: any) => {
    console.log("Nuevo familiar guardado:", nuevoFamiliar);
    console.log("Para el grupo:", grupoId);
...
function AddFamiliarPopup({ grupoId, planFijo, onClose, onSave }: AddFamiliarPopupProps) {
  ...
  const actualizarSituacion = (index: number, campo: 'situacion' | 'fechaFinalizacion', valor: string) => {
    setSituaciones(prev => {
      const nuevasSituaciones = [...prev];
      nuevasSituaciones[index] = { ...nuevasSituaciones[index], [campo]: valor };
      return nuevasSituaciones;
    });
  };
